...
public class Main_bj_G1_20304_비밀번호제작 {
	 ...
	 * 
	 * 최초에 해커들이 사용한 숫자를 입력받으면서 정답 배열에 0으로 넣는다.
	 * 이후 해커들이 사용한 숫자자와의 안전도가 1인 숫자들 ( 2, 1, 7번 인덱스 )에 1을 넣는다.
	 * 
	 * 이후 연산을 하여 안전도를 1씩 늘려가며 구한다.
	 * 최초 연산에서 도출된 안전도가 1인 2, 1, 7을 큐에 순서대로 큐에 넣는다.
	 * 이후 큐에서 빼내어 다시 1과 XOR 연산을 하면 안전도가 2인것을 구할수 있다.
	 * !!!! 다음줄부터 많이 혼란이 올 수 있다고 생각한다!
	 * 안전도가 1인것들을 큐에서 꺼내서 2인것을 구하는데, 이미 배열에 값이 들어가 있는 숫자가 연산결과로 나올것이다.
	 * 우리가 구하고자 하는 것은 해커가 사용한 패스워드와 XOR연산해서 구할 안전도이므로,
	 * 이미 구해진것들과 중복되는 연산결과는 저장대상에서 제외한다.
	 * 다시 설명하자면, 안전도가 2인것을 구했는데 이미 배열에 값이 들어가 있는 결과는 스킵한다.
